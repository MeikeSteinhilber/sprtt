...
The analyses below use the following specifications:
 ...
 - $\beta$ = `r pick_beta` (target power = $1-\beta$)
...
<!-- It is not a mistake that the code is here two times - there is a dependence in the simulated power as it requires N_max.  -->
...
table_cases <- df_all %>% 
  ...
  group_by(f_simulated, f_expected, power) %>%
  mutate(
    simulated_power = mean(if_else(f_simulated != 0 & decision  == "accept H1" & n_max >= n, 1, 0)),
    .after = power) %>%
  summarize(
    simulated_power = mean(simulated_power), # problem depends on N_max(decision rate)
    decision_error_rate = mean(decision_error),
    fix_n = as.integer(mean(fix_n)),
    mean_n = as.integer(round(mean(n))),
    median_n = as.integer(median(n)),
...
caption_html <- glue(
  ...
  "&beta; = {pick_beta}, &alpha; = 0.05, decision rate = {pick_decision_rate}%, groups = {pick_k_groups}"
)
...
::: {.alert .alert-info}
<!-- In short, to achieve a decision rate of `r pick_decision_rate*100`%, it is  -->
<!-- <span style="color: #2c3e50; font-weight: bold;"> -->
<!-- recommended to plan for a maximum sample size of $N_{\text{max}} = `r n_max`$. -->
<!-- </span><br> -->
<!-- With this $N_{\text{max}}$, the sequential ANOVA reaches a decision `r pick_decision_rate*100`% of the time, even in the worst case. -->
<!-- While $N_{\text{max}}$ provides a safe guard for the worst case, the <span style="color: #2c3e50; font-weight: bold;">median sample size is $N_{\text{median}} =$ `r n_median_grand`</span> in this scenario. -->
...
<span style="color: #2c3e50; font-weight: bold;">the median sample size is only $N_{\text{median}} =$ `r n_median_grand`</span>, meaning most of the time studies terminate much earlier.
:::
...
```{r}
table_cases %>%
  select(-grand_median_n) %>% 
  ungroup() %>% 
  mutate(decision_error_rate = round(decision_error_rate*100,1)) %>% 
  mutate(simulated_power = if_else(simulated_power == 0, "-", as.character(round(simulated_power*100,1)))) %>%
  gt() %>%
  tab_header(
    title = gt::html("<i>Summary table</i>"),
    subtitle = gt::html(glue("<i>f</i><sub>exp</sub> = {pick_f_expected}, &alpha; = 0.05, &beta; = {pick_beta}, decision rate = {pick_decision_rate}%, groups = {pick_k_groups}"))
  ) %>%
  ...
  cols_label(
    ...
    simulated_power     = gt::html("Power<sub>sim</sub>%"),
  ...
  fmt_number(
   ...
   tab_footnote(
    footnote = html(glue("simulated power to detect an effect which includes not only the type II error but also non-decisions")),
    locations = cells_column_labels(columns = simulated_power)
  ) %>% 
   tab_footnote(
...
table_cases <- df_all %>% 
  ...
  mutate(
    ...
    .after = power) %>%
  summarize(
    simulated_power = mean(simulated_power),# depends on N_max(decision rate)
    decision_error_rate = mean(decision_error),
    fix_n = as.integer(mean(fix_n)),
    mean_n = as.integer(round(mean(n))),
    median_n = as.integer(median(n)),
...
table_cases %>%
  ...
  tab_header(
    title = gt::html("<i>Summary table</i>"),
    subtitle = gt::html(glue("<i>f</i><sub>exp</sub> = {pick_f_expected}, &alpha; = 0.05, &beta; = {pick_beta}, groups = {pick_k_groups}"))
  ) %>%
  # center all data columns
  cols_align(align = "center", columns = everything()) %>%
  # center column labels and the heading
...
Notice how detection efficiency varies depending on whether the true effect is larger, equal to, or smaller than your expected effect.
