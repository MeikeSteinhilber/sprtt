...
author: "<i>created by the</i> sprtt <i>package</i>"
output:
  html_document:
    theme: flatly
    toc: true
...
<br><br>
This report summarizes a simulation with `r n_rep` iterations per condition.
...
This report contains multiple helpful aspects in order to plan a study using a sequential ANOVA, where the final sample size is unknown beforehand.
The first chapter visualizes the consequence of picking a specific effect size of interest ($f_{exp}$) in the context of multiple true effects ($f_{true}$).
This information is not specific to sequential testing, but is essential in choosing the correct expected efefct size.

The second chapter, contains the core information to plan a sequential study. In there, just two cases are taken into consideration - the case where the effect is exactly as specified ($f_{exp} = f_{true}$), or the true effect is zero ($f_{true} = 0$).

For more advanced users or curious readers, the third chapter contains extended plots and tables with more diverse true effect sizes.

<br>

## Influence of $f_{exp}$ on the power curve
...
df_all %>% 
  ...
  geom_point(aes(x = f_simulated, y = fix_n),
             ...
             color = red,
             size = 11) +
  geom_point(aes(x = f_simulated, y = mean(mean_n)),
             position = position_dodge(0.9),
             shape = 15,
             color = dark_blue,
             size = 8) + 
