...
if (is.null(df_all)) {df_all <- readRDS("C:/Users/msteinhi/GitHub/sprtt_plan_sample_size/meta_data/sprt_tool_df_all_10000.rds")}
...
recommendation <- table_cases %>%
  filter(.data[[pick_decision_rate_var]] == max(.data[[pick_decision_rate_var]]))

n_mean <- round(mean(table_cases$mean_n), 0)
n_median_grand <- table_cases$grand_median_n[1]
n_max <- recommendation[[pick_decision_rate_var]]
```
