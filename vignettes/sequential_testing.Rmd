...
The `sprtt` package is the implementation of **s**equential **p**robability **r**atio **t**ests using the associated **t**-statistic (**sprtt**). This vignette describes the theoretical background of these tests.
...
Basically it is not necessary to perform an analysis after each data point --- several data points can also be added at once. However, this affects the sample size (N) and the error rates [@stefan2020].
...
To eventually calculate the LR of the sequential t-test fortunately only the current t~*m*~-statistic is necessary. @rushton1950a showed that an SPRT can be performed by simply considering the ratio of probability densities for the most recent t~*m*~ statistic under the alternative and null hypothesis at any *m-*th stage. Thus, the test statistic for a one and two-sided sequential t-test can be calculated as follows:
...
To account for the fact that the algebraic sign is unknown in a two-sided test, the t-value is squared [@stefan2020].
