...
To use the `sprtt` package is must be installed and loaded. The latest release version of the package can be installed directly from CRAN or the package can be installed in the latest development version from GitHub. More information for the installation can be found on the home page.
...
The `seq_ttest()` function works similar to the `t.test()` function from the `stats` package if one is familiar with that already.

Sequential t-tests need some specification from the user:
...
If you want to perform a two-sample test, you need to use the `y` argument in addition to `x`. If the data are stored in a data frame, you need the `$` operator, to get access to the variables.
...
The `na.rm` argument defines the handling of missing values. If set to `TRUE` (default value), it will remove all missing values automatically. If this behavior is not wanted, the `na.rm` argument has to be set to `FALSE`. If missing values are then discovered, an error is triggered.
...
The simplest way to get the results is to run the `seq_ttest()` function. It will print the results automatically in the console. The verbose argument specifies how much information you want to be shown.

The recommended way thou is to save the results in an object (e.g "results"). Doing so allows running further calculations with it afterward. It is important to keep in mind, that the output object will be an S4 class. Therefore the access operator is the `@` sign or the `[]` brackets.
