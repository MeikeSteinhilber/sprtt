...
Basically one has two different possibilities, how to use the function: the `numeric` or the `formula` input option.

The `numeric` input is used if each group has its own variable. If you want to perform a one-sample test you have only one group and therefore only one variable. The variables can exist directly in the global environment or are stored in a data frame.

The `formula` input is used if you have both groups in one variable and you have a second variable which indicates the group membership.

#### numeric input  {style="color: blue"}

```{r}
# simulate data
female <- rnorm(n = 15, mean = 1.65, sd = 0.15)
male <- rnorm(n = 15, mean = 1.75, sd = 0.15)
df <- data.frame(female, male)

# sequential t-test (global variables)
seq_ttest(female, male, d = 0.3, verbose = verbose)

# sequential t-test (data frame)
seq_ttest(df$female, df$male, d = 0.3, verbose = verbose)

```

#### formula input {style="color: blue"}

```{r}
# simulate data
height <- rnorm(n = 30, mean = 1.70, sd = 0.15)
sex <- as.factor(rep(c("female", "male"), 15))
df <- data.frame(height, sex)

# sequential t-test (global variables)
seq_ttest(height ~ sex, d = 0.3, verbose = verbose)

# sequential t-test (data frame)
seq_ttest(height ~ sex, data = df, d = 0.3, verbose = verbose)
```
