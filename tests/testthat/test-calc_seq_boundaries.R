context("calc_seq_boundaries")

test_that("calc_seq_boundaries: ", {
  power <- 0.95
  alpha <- 0.05
  boundaries <- calc_seq_boundaries(power, alpha)
  boundaries_log <- calc_seq_boundaries(power, alpha, log = TRUE)
